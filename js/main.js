...
function setLista(lista) {
  ...
  for (var key in lista) {
    tabela += '<tr><td>' + formataDescript(lista[key].descript) + '</td><td>' + lista[key].quantidade + '</td><td>' + formatoValor(lista[key].valor) + '</td><td><button class="btn btn-default" onclick= "setAtt(' + key + ');" >Editar</button>  <button class="btn btn-default" onclick= "deleteAtt(' + key + ');" >Exlcuir</button> </td>';
  }
  tabela += '</tbody>';
...
function erro() {
  ...
  if (valor === "") {
    ...
    erros += 'O campo de valores deve possir números!';
  }
...
function delelaLista() {
  if (confirm('Dejesa mesmo apagar esta lista ?')) {
    list = [];
    setLista();
  }
}
